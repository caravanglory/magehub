...
skills/module/my-skill/
  ...
  SKILL.md             # Generated by npm run skills:sync
...
`SKILL.md` is generated for compatibility with the open `skills` CLI. Do not
edit it by hand; update `skill.yaml` or referenced Markdown/example files, then
run `npm run skills:sync`. Use `npm run skills:check` before committing to catch
drift.
