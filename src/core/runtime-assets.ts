...
function resolveAssetRoot(): string {
  ...
  if (existsSync(path.join(directRoot, 'schema')) && existsSync(path.join(directRoot, 'templates'))) {
    return directRoot;
  }

  const distFallbackRoot = path.resolve(moduleDir, '..');
  if (existsSync(path.join(distFallbackRoot, 'schema')) && existsSync(path.join(distFallbackRoot, 'templates'))) {
...
export function isPathInsideProject(rootDir: string, targetPath: string): boolean {
  const normalizedRoot = path.resolve(rootDir) + path.sep;
  const normalizedTarget = path.resolve(targetPath);

  return normalizedTarget === path.resolve(rootDir) || normalizedTarget.startsWith(normalizedRoot);
}
